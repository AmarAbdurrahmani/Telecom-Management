...
// roles: which roles can see this item (undefined = all staff)
const NAV_ITEMS = [
  {
  ...
  {
    path: '/kontratat', label: 'Kontratat', roles: ['admin','tl','sv'],
  ...
  {
    path: '/lifecycle', label: 'Mirëmbajtje', roles: ['admin','tl','sv'],
  ...
  {
    path: '/faturat', label: 'Faturat', roles: ['admin','tl','sv'],
  ...
  {
    path: '/paketat', label: 'Paketat', roles: ['admin','tl'],
  ...
  {
    path: '/pajisjet', label: 'Pajisjet', roles: ['admin','tl'],
  ...
  {
    path: '/numrat-telefonit', label: 'Numrat', roles: ['admin','tl'],
    icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>,
  },
  {
    path: '/sherbimet-shtesa', label: 'Shërbimet', roles: ['admin','tl'],
  ...
  {
    path: '/infrastruktura', label: 'Infrastruktura', roles: ['admin','tl'],
...
const STAFF_ITEMS = [
  {
    path: '/users', label: 'Menaxhimi i Stafit', roles: ['admin'],
...
function SidebarContent({ onNavClick }) {
  ...
  const visibleNav   = NAV_ITEMS.filter(item => !item.roles || item.roles.includes(user?.roli));
  const visibleStaff = STAFF_ITEMS.filter(item => !item.roles || item.roles.includes(user?.roli));
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <nav className="flex-1 py-3 px-3 overflow-y-auto scrollbar-hide">
        ...
        <div className="space-y-0.5">
          {visibleNav.map((item) => <NavItem key={item.path} item={item} onClick={onNavClick} />)}
        </div>
        ...
        {visibleStaff.length > 0 && (
          <>
            ...
            <div className="space-y-0.5">
              {visibleStaff.map((item) => <NavItem key={item.path} item={item} onClick={onNavClick} />)}
            </div>
          </>
        )}
      </nav>
